...
여러 명이 만날 때 출발지와 원하는 장소 카테고리를 기준으로 중간 약속 장소를 추천하는 Spring Boot 웹 서비스입니다.

## 주요 기능

- 참가자 2명부터 8명까지 출발 좌표 입력
- 카페, 맛집, PC방, 노래방, 술집, 스터디룸, 영화관, 볼링장 등 여러 카테고리 선택
- 선택한 카테고리별 추천 장소 Top 3 제공
- 평균 이동거리, 최대 이동거리, 거리 편차, 예상 이동시간, 공정성 표시
- 추천 기준 선택: 균형형, 짧은 이동 우선, 공정성 우선
- Kakao Local API 기반 실제 장소 검색
- Google Places API 기반 평점, 리뷰 수, 리뷰 문구 보강
